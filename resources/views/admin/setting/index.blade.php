    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-6">
            ...
            <div class="flex justify-between items-center">
                ...
                <a href="/" target="_blank" class="bg-white/10 hover:bg-white/20 px-4 py-3 rounded-2xl font-bold">
                    Lihat Website
                </a>
            </div>
            ...
            <form method="POST" action="{{ route('admin.setting.update') }}" enctype="multipart/form-data"
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-3xl p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <input type="text" name="site_tagline" value="{{ $setting->site_tagline ?? '' }}"
                            class="bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3" placeholder="Tagline">
                    </div>

                    <textarea name="site_description" rows="3"
                        class="w-full mt-4 bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3" placeholder="Deskripsi Website">{{ $setting->site_description ?? '' }}</textarea>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div class="bg-slate-800 border border-slate-700 rounded-2xl p-4">
                            <label class="block text-sm text-slate-400 mb-2">Logo Website</label>

                            @if ($setting->site_logo ?? false)
                                <img src="{{ asset($setting->site_logo) }}" class="h-16 mb-3 rounded-xl bg-white p-2">
                            @endif
                        ...
                        <div class="bg-slate-800 border border-slate-700 rounded-2xl p-4">
                            <label class="block text-sm text-slate-400 mb-2">Favicon</label>

                            @if ($setting->site_favicon ?? false)
                                <img src="{{ asset($setting->site_favicon) }}"
                                    class="h-10 mb-3 rounded-xl bg-white p-2">
                            @endif
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-3xl p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        @foreach ([['primary_color', 'Primary / Tombol Utama', '#2563eb'], ['secondary_color', 'Secondary / Gradasi', '#4f46e5'], ['base_color', 'Base Background', '#020617'], ['surface_color', 'Surface / Glass', '#0f172a'], ['card_color', 'Card Background', '#111827'], ['text_color', 'Text Utama', '#ffffff'], ['muted_text_color', 'Text Abu-Abu', '#94a3b8']] as $color)
                            <div class="bg-slate-800 border border-slate-700 rounded-2xl p-4">
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <input type="text" value="{{ $setting->{$color[0]} ?? $color[2] }}" readonly
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-3xl p-6">
                    ...
                    <textarea name="hero_subtitle" rows="4"
                        class="w-full mt-4 bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3" placeholder="Subjudul Hero">{{ $setting->hero_subtitle ?? '' }}</textarea>
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-3xl p-6">
                    ...
                    <textarea name="about_description" rows="4"
                        class="w-full mt-4 bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3" placeholder="Deskripsi Tentang">{{ $setting->about_description ?? '' }}</textarea>
                </div>
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-3xl p-6">
                    ...
                    <textarea name="footer_text" rows="2"
                        class="w-full mt-4 bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3" placeholder="Footer">{{ $setting->footer_text ?? '' }}</textarea>
                </div>
